...
1. *No dependencies*: CamlWriter was created without using external dependencies.
2. *Terminal Setup*: CamlWriter processes byte by byte having set the terminal to `raw mode` ([more info](https://en.wikipedia.org/wiki/POSIX_terminal_interface)).
3. *Terminal Interface*: CamlWriter is designed to be used entirely within a terminal environment, making it suitable for users who prefer working with text editors from the command line.
4. *Basic Text Editing*: Users can create, open, edit, and save plain text files using standard text editing commands.
