## M09 – Physik-Projekt: Kombination mehrerer Themen
Der Projekt-Sketch ist ein interaktives Template, bei dem zwischen Kinematik, Wellen und Optik umgeschaltet werden kann. Jede Szene zeigt eine einfache Visualisierung eines physikalischen Prinzips. Der Sketch dient als Ausgangspunkt für eigene Erweiterungen oder Präsentationsprojekte.
## Demo-Sketch (komplett)

```javascript
// M09a: Physik-Projekt – Projektschablone
// Vorlage mit UI: Wähle Themenmodus und bearbeite Parameter.
let mode=0; // 0 Kinematik, 1 Wellen, 2 Optik
function setup(){ createCanvas(800,450); textFont('monospace'); }
function draw(){
  background(252);
