...
tags:
  - "clippings"
---
/ **[helix](https://github.com/helix-editor/helix)** Public
...
Note: `v` is used along with `gl` (go to line end), because [`gl` does not select text](https://github.com/helix-editor/helix/issues/1630).`t<ret>` selects "'til" the newline represented by `<ret>`.
...
Unlike Vim, under Helix, the cursor shape is the same (block) in insert mode and normal mode by default. This can be adjusted in configuration:
